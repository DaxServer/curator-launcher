...
```sh
toolforge webservice buildservice --mount=none start --image tool-launcher/tool-launcher:latest
```

### DragonflyDB worker

```sh
toolforge jobs run dragonfly --image tool-launcher/tool-launcher:latest --command "dragonfly-launcher" --continuous --emails onfailure
```
